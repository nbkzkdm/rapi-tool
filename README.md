...
主用途は **手元の単体テスト**（必要なら軽い結合）です。  
Prism / WireMock の代替ではなく、**薄い・速い・CLI 中心**の別レーンです。

| 向く | 向かない（他ツール向き） |
|------|--------------------------|
| OpenAPI が無い／薄いときの数本モック | 契約検証・仕様どおりのモック → **Prism** |
| 条件分岐・埋め込み・遅延をすぐ試す | 録画再生・障害注入・複雑なマッチ → **WireMock** |
| 依存を抑えてローカルで即起動 | チーム共通の本格モック基盤 |
